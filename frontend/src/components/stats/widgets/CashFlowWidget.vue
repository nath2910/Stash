...
async function load() {
  ...
  try {
    const [k, s] = await Promise.all([
      StatsServices.kpi('cashAvailable', props.from, props.to, props.categories),
      StatsServices.series('cashAvailable', props.from, props.to, props.bucket, props.categories),
    ])
    if (id !== req) return
