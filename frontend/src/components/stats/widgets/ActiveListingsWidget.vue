...
async function load() {
  ...
  try {
    const [k, s] = await Promise.all([
      StatsServices.kpi('activeListings', props.from, props.to, props.categories),
      StatsServices.series('activeListings', props.from, props.to, props.bucket, props.categories),
    ])
    if (id !== req) return
